  ...
  return (
    <section className="py-20 sm:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-14 grid gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/5 sm:grid-cols-2">
          {items.map((row, i) => (
            <div
              key={i}
              className="grid grid-cols-1 sm:grid-cols-2 bg-[var(--background)] sm:col-span-2"
            >
              <div className="flex items-start gap-3 border-b border-white/5 p-5 sm:border-b-0 sm:border-r">
                <span className="mt-1 grid h-6 w-6 shrink-0 place-items-center rounded-full bg-rose-500/15 text-rose-300">
                  ✕
                </span>
                <p className="text-white/70 line-through decoration-rose-400/50">
                  {row.x}
                </p>
              </div>
              <div className="flex items-start gap-3 p-5">
                ...
                <p className="font-semibold">{row.w}</p>
              </div>
            </div>
          ))}
        </div>
