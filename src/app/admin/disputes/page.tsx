import Link from "next/link";
import { redirect, notFound } from "next/navigation";
import { Gavel } from "lucide-react";
import { createClient } from "@/lib/supabase/server";
import { FeedHeader } from "@/app/feed/_components/FeedHeader";
import { fetchOpenDisputes } from "@/app/disputes/actions";
import { ResolveForm } from "./_components/ResolveForm";

export const dynamic = "force-dynamic";

export default async function DisputesAdminPage() {
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
  if (!user) redirect("/login?next=/admin/disputes");

  const { data: profile } = await supabase
    .from("profiles")
    .select("display_name, role")
    .eq("id", user.id)
    .maybeSingle();
  if (profile?.role !== "owner" && profile?.role !== "admin") notFound();

  const disputes = await fetchOpenDisputes();

  return (
    <div className="min-h-screen bg-[var(--bg)] text-[var(--fg)]">
      <FeedHeader
        displayName={profile?.display_name ?? "founder"}
        role={profile?.role ?? "user"}
      />
      <main className="mx-auto max-w-2xl px-4 sm:px-6 py-6 sm:py-8 space-y-5">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight flex items-center gap-2">
            <Gavel className="h-5 w-5" />
            Dispute queue
          </h1>
          <p className="font-mono text-xs text-[var(--fg-subtle)] mt-1">
            Owner-only. Decide each dispute for the seeker (refund) or for the
            provider (release).
          </p>
        </div>

        {disputes.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-[var(--border)] bg-[var(--surface-1)]/40 p-8 text-center">
            <p className="text-[var(--fg-muted)]">No open disputes.</p>
          </div>
        ) : (
          <ul className="space-y-4">
            {disputes.map((d) => (
              <li
                key={d.id}
                className="rounded-2xl border border-amber-400/30 bg-amber-400/[0.05] p-5 space-y-3"
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="font-mono text-[10px] uppercase tracking-wider text-amber-400">
                      ${d.amount.toLocaleString()} · opened by{" "}
                      <Link
                        href={`/u/${d.opener_id}`}
                        className="underline hover:no-underline"
                      >
                        {d.opener_name}
                      </Link>
                    </p>
                    <p className="font-mono text-[10px] text-[var(--fg-subtle)] mt-0.5">
                      seeker:{" "}
                      <Link href={`/u/${d.seeker_id}`} className="text-[var(--fg-muted)] hover:text-[var(--fg)]">
                        {d.seeker_id.slice(0, 8)}…
                      </Link>{" "}
                      · provider:{" "}
                      <Link href={`/u/${d.provider_id}`} className="text-[var(--fg-muted)] hover:text-[var(--fg)]">
                        {d.provider_id.slice(0, 8)}…
                      </Link>{" "}
                      · {new Date(d.created_at).toLocaleString()}
                    </p>
                  </div>
                </div>
                <p className="text-sm text-[var(--fg)] whitespace-pre-wrap">{d.reason}</p>
                <ResolveForm disputeId={d.id} />
              </li>
            ))}
          </ul>
        )}
      </main>
    </div>
  );
}
